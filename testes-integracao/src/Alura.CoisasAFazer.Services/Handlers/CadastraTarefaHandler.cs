...
namespace Alura.CoisasAFazer.Services.Handlers
{
    public class CadastraTarefaHandler
    {
        ...
        // public CadastraTarefaHandler() //original
        public CadastraTarefaHandler(IRepositorioTarefas repositorio, ILogger<CadastraTarefaHandler> logger)
        {
            //_repo = new RepositorioTarefa(); // original, foi tirado pois depende de BD
            //_logger = new LoggerFactory().CreateLogger<CadastraTarefaHandler>();
            _repo = repositorio;
            _logger = logger;
        }

        //public void Execute(CadastraTarefa comando) //comando original
        public CommandResult Execute(CadastraTarefa comando)
        {
            try
            {
                ...
                return new CommandResult(true); //linha adicionada para o teste QuandoExceptionForLancadaResultadoIsSuccessDeveSerFalso()
            }
            catch (Exception e)
            {
